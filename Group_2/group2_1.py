...
while(1):
    ...
    for dir in dirlist:
        ...
        cmd = "hadoop fs -cat " + dir + "/*part* > /root/group2_1.csv"
        os.system(cmd)

        if(total == len(dirlist) or 1 == total or (last - total) >= 4):
            last = total
            print("Importing ...")
            stmnt = "truncate results2_1;\n"
            cmdpipe.stdin.write(stmnt)
            stmnt = "COPY results2_1 (origin, delayavg, carrier) FROM '/root/group2_1.csv';\n"
            cmdpipe.stdin.write(stmnt)
        ...
        print("Removing: " + "/root/group2_1.csv")
        cmd = "rm -f /root/group2_1.csv"
