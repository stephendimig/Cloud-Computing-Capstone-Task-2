...
while(1):
    ...
    for dir in dirlist:
        ...
        cmd = "hadoop fs -cat " + dir + "/*part* > /root/group2_2.csv"
        os.system(cmd)

        if(total == len(dirlist) or 1 == total or (last - total) >= 4):
            last = total
            print("Importing ...")
            stmnt = "truncate results2_2;\n"
            cmdpipe.stdin.write(stmnt)
            stmnt = "COPY results2_2 (origin, delayavg, dest) FROM '/root/group2_2.csv';\n"
            cmdpipe.stdin.write(stmnt)
        ...
        print("Removing: " + "/root/group2_2.csv")
        cmd = "rm -f /root/group2_2.csv"
