...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: originanddest.py <zk> <topic>", file=sys.stderr)
        exit(-1)

    sc = SparkContext(appName="PythonOriginAndDest")
    ssc = StreamingContext(sc, 1)
    ssc.checkpoint("storage")
    # RDD with initial state (key, value) pairs
    initialStateRDD = sc.parallelize([((u'origin', u'dest'), (2147483647, 1))])
    
    zkQuorum, topic = sys.argv[1:]
    kvs = KafkaUtils.createStream(ssc, zkQuorum, "spark-streaming-consumer", {topic: 1})

    lines = kvs.map(lambda x: x[1])
    data = lines.map(lambda line: ((line.split(',')[3], line.split(',')[4]), (0 if False == line.split(',')[11].isdigit() else int(line.split(',')[11]), 1))) \
               .updateStateByKey(updateFunc)
        

    def process(time, rdd):
        uri = "hdfs://sandbox.hortonworks.com/user/root/output/group2_2." + str(getInvokeNum())
    ...
    def rdd2csv(p):
        ...
        for mtuple in mlist:
            if(count < 10):
                ...
                break
            rlist.append(str(origin + "," + str(mtuple[1]) + "," + mtuple[0]))
        return rlist
