...
if __name__ == "__main__":
    ...
    initialStateRDD = sc.parallelize([((u'origin', u'carrier'), (2147483647, 1))])
    
    zkQuorum, topic = sys.argv[1:]
    kvs = KafkaUtils.createStream(ssc, zkQuorum, "spark-streaming-consumer", {topic: 1})

    lines = kvs.map(lambda x: x[1])
    data = lines.map(lambda line: ((line.split(',')[3], line.split(',')[6]), (0 if False == line.split(',')[11].isdigit() else int(line.split(',')[11]), 1))) \
               .updateStateByKey(updateFunc)
        

    def process(time, rdd):
        uri = "hdfs://sandbox.hortonworks.com/user/root/output/group2_1." + str(getInvokeNum())
